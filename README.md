...
### 表4: 多步潜在想象强化学习 (Stage 1) 🔄 进行中

**目标**: 验证解耦抽象能否支持潜在空间策略学习

**Pilot结果** (Seed 7, Horizon=5):

| Condition | Latent Return ↑ | Collision Rate ↓ | Stability |
|-----------|----------------|-----------------|-----------|
| bc | 10.85 | 56.0% | 0.086 |
| wm_object | 26.85 | 51.7% | 0.211 |
| wm_decoupled | 9.88 | 48.4% | 0.289 |

**待完成**: 3 seeds × 6 conditions 完整实验

### 表5: 反应式闭环实验 (Stage 2) 📋 规划中

**目标**: 验证"反应式训练很重要"

| Setting | Merge ↑ | Yield ↑ | Cut-in ↑ | Near Collision ↓ |
|---------|---------|---------|----------|------------------|
| Replay+Replay | - | - | - | - |
| Replay+Reactive | - | - | - | - |
| Reactive+Reactive (Ours) | - | - | - | - |

### 表6: 高保真评估 (Stage 3) 📋 规划中
...
| Stage | 主题 | 状态 | 完成度 |
|-------|------|------|--------|
| **Stage 0** | 表示充分性消融 | ✅ 完成 | 100% |
| **Stage 1** | 多步潜在想象RL | 🔄 进行中 | ~30% |
| **Stage 2** | 反应式闭环评估 | 📋 规划 | 0% |
| **Stage 3** | 高保真评估 | 📋 规划 | 0% |
...
### 短期 (1-2周)
1. 完成 Stage 1 完整实验 (3 seeds × 6 conditions)
2. 解决 collision rate 偏高问题 (~50%)
3. 分析想象 rollout 稳定性

### 中期 (1个月)
1. 实现 Stage 2: 反应式闭环评估
2. 集成 nuPlan closed-loop 评估器
3. 添加交互场景的 merge/yield/cut-in 指标

### 长期 (论文投稿前)
1. Stage 3: 高保真评估 (CARLA/3DGS)
2. 对比基线: BC、Model-free RL、Holistic Dreamer
3. 论文写作与可视化
